...
* You must install only one version of Java on your Unified Manager host; otherwise, the system may not be able to determine which version to use. See https://kb.netapp.com/data-mgmt/AIQUM/AIQUM_Kbs/Active_IQ_Unified_Manager_installation_fails_due_to_multiple_Java_versions[Active IQ Unified Manager installation fails due to multiple Java versions].
