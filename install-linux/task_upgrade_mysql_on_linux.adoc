...
[.lead]
You can upgrade to a newer version of MySQL on the Linux server on which Unified Manager is installed to obtain fixes for security vulnerabilities. If the installed version of MySQL on your system is earlier than 8.0.28, the Unified Manager 9.11 upgrade process automatically upgrades MySQL to 8.0.28. You should not run a standalone upgrade of MySQL from an earlier version to 8.0.28.

*What you'll need*

You must have root privileges for the Linux system on which Unified Manager is installed.

You can upgrade the base version of MySQL 8.0.28 to later versions for only minor updates.
