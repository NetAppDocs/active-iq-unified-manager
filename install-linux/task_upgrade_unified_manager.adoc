...
[.lead]
You can upgrade from Unified Manager 9.9 or 9.10 to 9.11 by downloading and running the installation file on the Linux platform.
...
[NOTE]
These steps need to be performed after step _5_ of the main procedure and following this, and run the `upgrade.sh` command to continue with step _6_.
...
. Change directories to the new directory on the installation system and run the following command for installing the missing packages, along with their dependencies.
+
`yum install *.rpm`
