...
. Download the Unified Manager Windows installation file or zip file to a target directory on the Windows system. If you downloaded the zip file, unzip the file to find the installation file. Right-click and run the Unified Manager installer executable (.exe) file as an administrator.
+
