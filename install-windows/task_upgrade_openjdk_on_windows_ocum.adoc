...
You can update OpenJDK releases within release families. For example, you can upgrade from OpenJDK 11.0.14 to OpenJDK 11.0.16, but you cannot update directly from OpenJDK 11 to OpenJDK 12.
...
+
For example, download `jdk-11.0.6_windows-x64_bin.zip from https://www.oracle.com/in/java/technologies/javase/jdk11-archive-downloads.html`.
+
[NOTE]
...
. Expand the `zip` file.
. Copy the directories and files from the resulting `jdk` directory (for example, `jdk-11.0.16` to the location where Java is installed. Example: `C:\Program Files\NetApp\JDK\`
