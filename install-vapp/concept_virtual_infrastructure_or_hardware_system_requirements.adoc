...
Additionally, there is a limit to the number of nodes that a single instance of Unified Manager can monitor before you install a second instance of Unified Manager. For more information, see link:http://www.netapp.com/us/media/tr-4621.pdf[Unified Manager Best Practices Guide]
...
//== Considerations for hardware version upgrade for virtual appliances
//NetApp recommends you to avoid manually upgrading the hardware version of your existing virtual appliance. In case you need to upgrade the hardware version, follow these steps:
//. Take a backup of your current system.
//. Deploy an updated version of the Unified Manager virtual appliance (vApp). This creates a new VM with a more recent hardware version.
//If you have the most recent version of the Unified Manager virtual appliance installed, you should wait for the next Unified Manager release before upgrading the hardware version.
//. Restore the backup on the new VM to retain the data.
