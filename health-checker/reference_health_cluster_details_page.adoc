 ...
 ** Logical Space Used
+
The real size of the data that is being stored on all aggregates on this cluster without applying the savings from using ONTAP storage efficiency technologies. This measurement does not include Snapshot copies.

 ** Data Reduction
+
Display the ratio without Snapshot copies and with two significant digits, for example, 1.8 to 1. This ratio is based on configured ONTAP storage efficiency settings.
