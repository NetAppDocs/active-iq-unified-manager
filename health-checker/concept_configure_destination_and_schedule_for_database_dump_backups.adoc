...
* If you find that the number of incremental backup files is getting too large for the space you have allocated for backups, you can create a new full backup periodically to replace the old full backup and all of its' child incremental files. As another option, you can back up using Snapshot copies if Unified Manager is installed on a Linux system.
