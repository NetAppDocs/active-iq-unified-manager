...
+
Controls the access to the default MySQL port 3306. For reasons of security, the access through this port is restricted only to localhost during a fresh installation of Unified Manager on Linux, Windows, and VMware vSphere systems. By using this option, you can make this port available to other hosts. If the access was enabled on multiple hosts previously, the same configuration is retained in an upgrade scenario. 
* *Exit*
+
