...
Periodically checks for clusters with licenses for anti-ransomware monitoring. Detects whether the volumes are in the `learning` mode of anti-ransomware monitoring for about 45 days, and determines the prospect of putting them in an active mode.
a|
...
Periodically checks for clusters with licenses for anti-ransomware monitoring. Detects repetitious notifications during active anti-ransomware monitoring for the volumes (for example, multiple warnings of potential ransomware attacks are returned over 30 days).
a|
Disable anti-ransomware monitoring for the volumes
a|
