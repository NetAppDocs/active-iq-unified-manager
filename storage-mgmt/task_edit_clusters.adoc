...
. In the *Edit Cluster* dialog box, modify the values as required.
    +
  If you have modified the details for a cluster running ONTAP 9.5 version or greater, or upgraded a cluster running ONTAP older version to higher version, you can view the certificate details for Mutual TLS communication by clicking *Certificate Details*. If the certificate is expired, click the *Regenerate* button to incorporate the new certificate.
. Click *Submit*.
