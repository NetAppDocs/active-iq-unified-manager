...
---
= Remove clusters from Active IQ Unified Manager
:icons: font
:imagesdir: ../media/
...
[NOTE]
====
You must remove a cluster from Unified Manager before decommissioning it. If the cluster is already offline or unreachable when you attempt to remove it, Unified Manager cannot communicate with the cluster to remove the application record and configured Event Management System (EMS) subscriptions. This will cause the removal process to fail or become unresponsive before completion.
====
