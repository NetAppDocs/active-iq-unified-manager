...
+
*Certificates for Mutual TLS communication*:Used during Mutual TLS communication between Unified Manager and ONTAP. It enables certificate-based authentication for a cluster, based on the ONTAP version. If the cluster running the ONTAP version is lower than the 9.5, certificate-based authentication is not enabled.
+
When a cluster is added to Unified Manager 9.12, the certificate creates automatically.Though, the certificate is not created automatically for a cluster, if you are updating an older version of Unified Manager to Unified Manager 9.12. However, you can create it by modifying and saving the cluster details. If the certificate expires, you should regenerate it to incorporate the new certificate. For more information about viewing and regenerating the certificate, see link:task_edit_clusters.adoc[Editing clusters]. 
+
[NOTE]
...
+
[NOTE] 
If you are adding a cluster to Unified Manager 9.12, see link:../storage-mgmt/task_edit_clusters.html#certificates-for-mutual-tls-communication[Certificates for Mutual TLS communication]
