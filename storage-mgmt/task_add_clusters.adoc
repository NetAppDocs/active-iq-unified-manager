...
====
* You should have the certificate-based authentication for a cluster, if you are upgrading from ONTAP older software version to 9.5 software version. You can enable it by editing the cluster details. For more information about editing the cluster details, see link:task_edit_clusters.html[Editing clusters].
+
The certificate-based authentication is enabled for ONTAP version 9.5 software or greater. If you add a cluster for ONTAP version 9.5 software or greater from the web UI, the certificate-based authentication enables automatically. If the certificate-based authentication is expired or about to expire for a cluster, you should regenerate it.
