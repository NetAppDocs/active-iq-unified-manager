...
---
= Add alerts in Active IQ Unified Manager
:icons: font
:imagesdir: ../media/
...
* Please note that if you add a Storage Virtual Machine (SVM) or a cluster to an alert's resources, child objects are not added automatically. For example, if you add a cluster to an alert's resources, the alert does not automatically add the nodes or LUNs.
